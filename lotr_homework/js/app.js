...
const keepItSecretKeepItSafe = () => {
  ...
  const $divRing = $('<div/>', { id: 'the-ring' });
  // 2. add the ring as a child of Frodo
  const $frodoLi = $('.hobbit').eq(0);
  $frodoLi.append($divRing);
...
const $divFellowship = $('<div/>', { id: 'the-fellowship'} );
  // 2. add an h1 with the text 'The Fellowship' to this new div
$divFellowship.append('<h1>The Fellowship');
  // 3. append the fellowship to middle-earth
$('#middle-earth').append($divFellowship);
  // 4. add the unordered lists of hobbits and buddies to 'the-fellowship'
  const $ulHobbits = $('#Rivendell ul');
  $('#the-fellowship').append($ulHobbits);
  const $ulBuddies = $('aside ul');
  $('#the-fellowship').append($ulBuddies);
};
...
const weWantsIt = () => {
  ...
  const $divGollum = $('<div/>', { class: 'gollum' });
   $('#Mordor').append($divGollum);
  // 2. Move the ring from Frodo and give it to Gollum
  const $divRing = $('#the-ring');
  $divGollum.append($divRing);
  // 3. Move Gollum into Mount Doom
  $("div:contains('moount-doom')").append($divGollum);
